...
| `planning-export.js` | classic | Read-only export van geplande activiteit (Zoho-tickets + handmatige afspraken) voor externe koppelingen (Base44), beveiligd met `PLANNING_EXPORT_API_KEY` |
...
- **`planning-export.js`-endpoint toegevoegd (2026-07-27)** — `GET /api/planning-export` geeft alle geplande activiteit (Zoho-tickets met ingevulde `interventieDatum` + handmatige afspraken) terug als platte JSON, voor de Base44-planningsapp van een collega. Spec: [`docs/superpowers/specs/2026-07-27-planning-export-integratie-design.md`](docs/superpowers/specs/2026-07-27-planning-export-integratie-design.md). Beveiligd met een gedeelde sleutel (`PLANNING_EXPORT_API_KEY`, `Authorization: Bearer <sleutel>`) — **die sleutel moet apart (buiten git, bv. via een beveiligd kanaal) met de collega gedeeld worden, niet in `.env.local.example` gezet**. De collega's app moet dit endpoint elke 3-5 minuten bevragen (polling, geen webhook).
  - Twee latente datamodel-bugs ontdekt tijdens deze implementatie, die het endpoint (bewust, defensief) gewoon negeert i.p.v. laat crashen: (1) `klantbeschikbaarheid.js`'s PUT-handler persisteert `duurOverride` nooit server-side (al gekend, zie `docs/superpowers/plans/2026-07-25-bug-fix-roadmap.md` regel 2211) — `planning-export.js`'s duur-override-logica is correct maar dus inert totdat dat gefixt is. (2) **Nieuw ontdekt:** `afspraken.js`'s PUT-handler bewaart het `adres`-veld van een handmatige afspraak niet (ondanks dat de client dat wel meestuurt), waardoor `bron:"handmatig"`-items in de export altijd `adres:null` hebben. Geen van beide is in deze taak gefixt (buiten scope) — wachten op Brents prioriteit.
