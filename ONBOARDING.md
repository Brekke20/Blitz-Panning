...
- **Bugfix-roadmap uitgevoerd (2026-07-25/26)** — alle 26 taken uit [`docs/superpowers/plans/2026-07-25-bug-fix-roadmap.md`](docs/superpowers/plans/2026-07-25-bug-fix-roadmap.md) zijn geïmplementeerd, individueel gereviewd, en door een finale whole-branch review + één fix-wave gehaald, in de worktree `worktree-bug-fix-roadmap` (nog niet gemerged naar `main`, nog niet gepusht). Volledig verloop (elke commit, elke review, elke fix-ronde) staat in `.claude/worktrees/bug-fix-roadmap/.superpowers/sdd/2026-07-25-bug-fix-roadmap/progress.md`.
  - **Opgeloste kritieke bevindingen:** rapport.js SSRF (netwerktoegang geblokkeerd in de PDF-render), path traversal via ticketId (5 bestanden), propose.js open mail-relay (recipient nu gevalideerd tegen het echte ticket), 4 debug-endpoints verwijderd, CORS-domein gefixt, prijzen.js/rapport-archief.js optimistic-locking, XSS-sweep op ticketvelden (meerdere rondes), route-leg-index/aanrijtijd/facturatie-bugs, feestdagen in autoplan, wizard-focus/handtekening-bugs, en meer — zie het rapport/de ledger voor het volledige overzicht.
  - **Nog open, aanbevolen als eerstvolgende kleine taak:** de finale review vond dat de escHtml-apostrof-fix (inline `onclick` → `data-*`+`addEventListener`, nodig omdat escHtml() geen bescherming biedt binnen een JS-string-context) maar op 3 van in totaal ~19 gelijkaardige plekken is toegepast. De overige ~16, waarvan minstens 5 met exact hetzelfde dreigingsmodel (ongeauthenticeerde blob-schrijfbare IDs: `removeLocalEvent`, `avRemoveException`, `openLocalEventDetail`, `prijsVoegOnderdeel`, de `updateKb`-cluster), staan nog open. Geen regressie t.o.v. voor deze branch, maar wel dezelfde kwetsbaarheidsklasse als wat al gefixt is — zie de gespawnde taak-suggestie of de ledger voor de volledige lijst.
  - **Secrets-rotatie (Zoho client-ID/secret, TomTom-key) blijft een manuele actie voor Brent**, buiten deze branch — de code-kant (`.env.local.example` schonen) is al gedaan.
